...
export function AuthScreen({ role, mode, redirectTo }: AuthScreenProps) {
  ...
  const alternatePath =
    mode === "login"
      ? ROUTES.AUTH.LOGIN[alternateRole]
      : ROUTES.AUTH.SIGNUP[alternateRole];
    ...
    <main className={styles.background} data-auth-mode={mode}>
      <section className={styles.panel} aria-labelledby="auth-title">
        ...
        >
          ...
          <p className={styles.roleSwitch}>
            ...
            <Link
              ...
              href={authHref(alternatePath, redirectTo)}
            >
              {isCustomer ? "기사님" : "일반 유저"} 전용 페이지
            </Link>
          </p>
        </header>
        <AuthController
        ...
        <Image
          ...
          src={`/images/auth/${isCustomer ? "customer" : "mover"}.png`}
          alt=""
          width={392}
          height={392}
        />
