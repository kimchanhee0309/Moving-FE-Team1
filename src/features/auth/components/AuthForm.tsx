...
import { AUTH_STYLES as styles } from "../auth.styles";
...
const FIELD_PLACEHOLDERS: Record<AuthField, string> = {
  name: "성함을 입력해 주세요",
  email: "이메일을 입력해 주세요",
  phone: "숫자만 입력해 주세요",
  password: "비밀번호를 입력해 주세요",
  passwordConfirm: "비밀번호를 다시 한번 입력해 주세요",
};
...
export function AuthForm({
  ...
  const unavailableMessage =
    mode === "login"
      ? "현재 로그인을 이용할 수 없습니다. 잠시 후 다시 시도해 주세요."
      : "현재 회원가입을 이용할 수 없습니다. 잠시 후 다시 시도해 주세요.";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!onSubmitValues) {
      setSubmitError(unavailableMessage);
      return;
    }
    ...
    try {
      ...
      setSubmitError(
        error instanceof ApiError && error.code === "EMAIL_ALREADY_EXISTS"
          ? "이미 가입된 이메일입니다. 로그인해 주세요."
          : error instanceof ApiError && error.status === 429
            ? "요청이 많습니다. 잠시 후 다시 시도해 주세요."
            : mode === "login"
              ? "로그인하지 못했습니다. 이메일·비밀번호와 계정 유형을 확인해 주세요."
  ...
  async function handleSocialLogin(provider: SocialProvider) {
    ...
    if (!onSocialLogin) {
      setSubmitError(
        "현재 SNS 로그인을 이용할 수 없습니다. 잠시 후 다시 시도해 주세요.",
      );
      return;
    }
    submitLock.current = true;
    ...
    try {
      ...
      setSubmitError(
        error instanceof ApiError && error.code === "OAUTH_NOT_CONFIGURED"
          ? "SNS 로그인 준비 중입니다. 이메일 로그인을 이용해 주세요."
          : "SNS 로그인에 실패했습니다. 다시 시도해 주세요.",
      );
    } finally {
      submitLock.current = false;
      setIsSubmitting(false);
    ...
    <div className={styles.content}>
      ...
      <p className={styles.accountSwitch}>
        ...
        <Link
          ...
          href={authHref(
            mode === "login"
              ? ROUTES.AUTH.SIGNUP[role]
              : ROUTES.AUTH.LOGIN[role],
            redirectTo,
          )}
        >
      ...
      <section className={styles.social} aria-label="SNS 로그인">
        <p className={styles.socialCopy}>SNS 계정으로 간편 가입하기</p>
        <div className={styles.socialButtons}>
          {SOCIAL_PROVIDERS.map(({ provider, label, image }) => (
            <button
              ...
              key={provider}
              type="button"
              aria-label={`${label}로 ${role === "CUSTOMER" ? "일반 유저" : "기사님"} 로그인`}
              disabled={isSubmitting}
