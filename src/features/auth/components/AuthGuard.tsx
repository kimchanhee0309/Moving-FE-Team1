...
export function AuthGuard({role,children}:{role:UserRole;children:ReactNode}) {
  ...
  const profileRegister=role==="MOVER"?ROUTES.MOVER.PROFILE.REGISTER:ROUTES.CUSTOMER.PROFILE.REGISTER;
  useEffect(()=>{
    if(!isPending&&!user&&!error)router.replace(login);
    else if(!isPending&&user?.role===role&&!user.profileCompleted&&path!==profileRegister)router.replace(profileRegister);
  },[isPending,user,error,login,path,profileRegister,role,router]);
  ...
  if(!user.profileCompleted&&path!==profileRegister)return <p role="status" className="p-8 text-center">프로필 등록 화면으로 이동하고 있습니다. <Link href={profileRegister} className="underline">프로필 등록</Link></p>;
  return children;
}
